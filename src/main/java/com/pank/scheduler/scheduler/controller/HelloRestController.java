...
//	private String validity;
	...
	private Map<String, Object> generateTokenJWT(@RequestBody String body) {
		...
		try {
			...
			String token = Jwts.builder().signWith(SignatureAlgorithm.HS256, secret).setIssuedAt(new Date(timestamp))
					.setExpiration(new Date(timestamp + validity)).claim("userid", has.get("userId"))
					.claim("password", has.get("password")).compact();
